...
async function seedPortalEstudiantes() {
  console.log('\n🎓 6. Creando Portal Estudiantes...');
  try {
    await PortalEstudiantes.destroy({ where: {} });

    const portalData = {
      titulo: 'Portal de Estudiantes',
      subtitulo: 'Accede a todos los servicios académicos',
      badgeTexto: 'Nuevo',
      parrafo1: 'Bienvenido al Portal Estudiantes de la Universidad Tecnológica de Tecamachalco. Aquí podrás acceder a todos los servicios académicos y administrativos que necesitas.',
      parrafo2: 'Consulta tus calificaciones, horarios, servicios escolares, biblioteca digital y mucho más desde un solo lugar.',
      parrafo3: 'Mantente conectado con tu universidad y gestiona todos tus trámites de forma rápida y segura.',
      imagenUrl: '/uploads/carreras/portadas/MODELO EDUCATIVO 2025.jpg',
      enlaceBoton: 'http://187.217.125.214/uttecam/acceso.asp',
      textoBoton: 'Acceder al Portal',
      activo: true
    };

    await PortalEstudiantes.create(portalData);
    console.log('   ✅ Portal Estudiantes creado');
  } catch (err) {
    console.error('   ❌ Error creando portal estudiantes:', err);
    throw err;
  }
}
